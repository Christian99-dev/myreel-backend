...
def print_database_contents(db: Session, show_tables: Dict[str, bool]):
    ...
    if show_tables.get('Slot', False):
        print("Slot Table:")
        slots = db.execute(select(model.Slot)).scalars().all()
    ...
    if show_tables.get('Song', False):
        print("Song Table:")
        songs = db.execute(select(model.Song)).scalars().all()
    ...
    if show_tables.get('Edit', False):
        print("Edit Table:")
        edits = db.execute(select(model.Edit)).scalars().all()
    ...
    if show_tables.get('Group', False):
        print("Group Table:")
        groups = db.execute(select(model.Group)).scalars().all()
    ...
    if show_tables.get('Invitation', False):
        ...
        invitations = db.execute(select(model.Invitation)).scalars().all()
    ...
    if show_tables.get('LoginRequest', False):
        ...
        login_requests = db.execute(select(model.LoginRequest)).scalars().all()
    ...
    if show_tables.get('OccupiedSlot', False):
        ...
        occupied_slots = db.execute(select(model.OccupiedSlot)).scalars().all()
