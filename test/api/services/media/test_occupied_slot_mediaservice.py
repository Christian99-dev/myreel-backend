...
def test_get_occupied_slot_file_found(media_access_memory):
    ...
    media_access_memory.save(file_name, "occupied_slots", b"dummy_slot_data")  # Speichere Dummy-Daten

    # Ruft die get-Funktion auf
    result = get(slot_id, media_access_memory)

    # Überprüfe, ob die zurückgegebenen Daten korrekt sind
    assert result == b"dummy_slot_data"

def test_get_occupied_slot_file_not_found(media_access_memory):
...
# create
def test_create_occupied_slot_file(media_access_memory):
    slot_id = 1  # Angenommene slot_id
    file_extension = 'mp4'  # Angenommene Dateierweiterung
    file_data = b"dummy_slot_data"

    # Ruft die create-Funktion auf
    location = create(slot_id, file_extension, file_data, media_access_memory)

    # Überprüfe, ob die Datei erfolgreich erstellt wurde
    expected_file_name = f"{slot_id}.{file_extension}"
    assert expected_file_name in media_access_memory.list("occupied_slots")

    # Überprüfe, ob die Datei die erwarteten Daten hat
    result = media_access_memory.get(expected_file_name, "occupied_slots")
    assert result == file_data

# remove
def test_remove_occupied_slot_file_found(media_access_memory):
    slot_id = 1  # Angenommene slot_id
    file_extension = 'mp4'  # Angenommene Dateierweiterung
    file_name = f"{slot_id}.{file_extension}"
    ...
    result = remove(slot_id, media_access_memory)

    # Überprüfe, ob die Datei erfolgreich entfernt wurde
    assert result is True
    assert file_name not in media_access_memory.list("occupied_slots")

def test_remove_occupied_slot_file_not_found(media_access_memory):
    slot_id = 9999  # Angenommene nicht existierende slot_id
...
# update
def test_update_occupied_slot_file(media_access_memory):
    slot_id = 1
    ...
    create(slot_id, file_extension, initial_data, media_access_memory)

    # Update die Datei
    update(slot_id, file_extension, update_data, media_access_memory)

    # Überprüfe, ob die Datei aktualisiert wurde
    retrieved_slot = get(slot_id, media_access_memory)
