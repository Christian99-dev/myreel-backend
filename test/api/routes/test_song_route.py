...
def test_get_song_not_found(app_client_filled: TestClient):
    response = app_client_filled.get("/song/get/999999")  
    assert response.status_code == 404
    assert response.json() == {"detail": "Song not found"}

def test_get_song_invalid_id(app_client_filled: TestClient):
    response = app_client_filled.get("/song/get/abc")  
